...
let command
const compiler = '--compiler="js:./server/build/babel-register-compiler.js"'
const dirs = '--migrations-dir=server/data/migrations'
const filename = `server/data/.migrate-${env}`
...
if (script === 'm:up') {
	...
	command = `migrate create ${args[1]} ${shared} --template-file server/utils/migration-template.js`
...
if (script.match(/^m:test:/)) {
	...
	// spawn('node -r esm server/data/migrations/1527054780080-add-repo-metadata.js', { stdio: 'inherit', shell: true })
} else {
	console.log(command)
...
function getMigration(direction) {
	...
	const migrations = globby.sync('./server/data/migrations/**/*.js').map(path => path.split('/migrations/')[1])
